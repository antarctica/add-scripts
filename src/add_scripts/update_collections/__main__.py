...
def get_revision_of_id(record: dict) -> Optional[str]:
    for aggregation in record['identification']['aggregations']:
        if 'association_type' in aggregation and aggregation['association_type'] == 'revisionOf':
            return aggregation['identifier']['identifier']
    return None
...
def update_current_collection(collection: dict, new_records: list[dict]) -> dict:
    ...
    for record in new_records:
        old_id = get_revision_of_id(record)
        if old_id is None:
    ...
    for aggregation in collection['identification']['aggregations']:
        if aggregation['association_type'] == 'isComposedOf' and aggregation["initiative_type"] == "collection" and aggregation['identifier']['identifier'] in old_new_ids:
...
def update_previous_collection(collection: dict, new_records: list[dict]) -> dict:
    old_ids = [get_revision_of_id(record) for record in new_records]
    ...
    collection['identification']['aggregations'] = new_aggregations + collection['identification']['aggregations']
...
def main():
    ...
    previous_collection = update_previous_collection(previous_collection, list(new_records.values()))
    ...
    current_collection = update_current_collection(current_collection, list(new_records.values()))
