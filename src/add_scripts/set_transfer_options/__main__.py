...
def process_artefacts(rows: list[dict]) -> dict[str, list[tuple[str, int, str]]]:
    ...
    for row in rows:
        ...
        artefacts[row["record_id"]].append(
            (row["media_type"], row["size_bytes"], row["access_url"])
        )

    return artefacts


def process_distribution_options(
    record: dict, artefacts: list[tuple[str, int, str]]
) -> dict:
    if len(artefacts) == 0:
        return record
